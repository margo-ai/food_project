...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  window.addEventListener('scroll', showModalByScroll); //  {once: true}  // чтобы наше событие выполнилось только один раз
}
...
function slider(_ref) {
  ...
  slider.style.position = 'relative'; // обёртка для всех точек
...
function timer(id, deadline) {
  ...
  function getTimeRemaining(endtime) {
    var t = Date.parse(endtime) - Date.parse(new Date()); // разница в миллисекундах

    var days = Math.floor(t / (1000 * 60 * 60 * 24)); // сколько суток осталось до окончания дедлайна
